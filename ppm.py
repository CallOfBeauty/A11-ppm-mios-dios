######################################################################
# Author: Ntentiad DImitrios, Rodolfo Alvarado
...
0 0 0 20 20 20 40 40 40 60 60 60 80 80 80 100 100 100 120 120 120 140 140 140'''
    ...
    def PPM_rotateclockwise(self):
        ...
        newpixellist=[]
        for i in range(len(pixellist[0])):
            row=[]
            for j in range(len(pixellist)):
                row.append(0)
            newpixellist.append(row)

        for row in range(len(pixellist)):
            length=(len(newpixellist[0])-1)-row
            for col in range(len(pixellist[row])):
                newpixellist[col][length]=pixellist[row][col]
    ...
    def PPM_make_blue(self):
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                newpixellist[row][col][0] = 0  # update green
                newpixellist[row][col][1] = 0  # update blue
                col += 1
            row += 1
        print(self.outasciifile + " output file turned red.")
        self.PPM_updatefrompixellist(newpixellist)  # This call will update all member attributes appropriately.
